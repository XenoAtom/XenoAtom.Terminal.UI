...
- **Modern control library** (60+ built-in controls):
  ...
  - Text editing: **TextBox**, **TextArea**, **CodeEditor**, **PromptEditor**, **MaskedInput**, **NumberBox** (undo/redo: `Ctrl+Z` / `Ctrl+R`, clipboard paste interception, auto-expanding editor height via `TextEditorAutoSizeMode.Height`, `CodeEditor` Find/Replace: `Ctrl+F` / `Ctrl+H`, Go To Line: `Ctrl+G`, configurable spaces/tabs indentation, `PromptEditor` completion gestures that can be rebound to preserve Tab focus traversal)
