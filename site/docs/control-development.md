...
- Bound generated properties are synchronized from their source on source writes; do not rely on getters to perform side effects.
- Generated `Func<T>` fluent overloads on `Visual` types install tracked computed configuration via dynamic updates; they are not synthetic property bindings.
...
### Use computed visual configuration instead of getter tricks
- If a visual property should follow other bindables, prefer the generated `Property(() => expr)` fluent overloads.
- Do not make public getters perform synchronization, clamping, or other visible writes.
- If a control needs derived read-only state, model it explicitly inside the control instead of relying on getter-time updates.
...
- Do not write a clamped public bindable back from `PrepareChildren`, `MeasureCore`, `ArrangeCore`, or `RenderOverride`; compute a safe local value and use that instead.
