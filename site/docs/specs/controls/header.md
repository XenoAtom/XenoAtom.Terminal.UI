...
This document captures design and implementation notes for `Header`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `Header` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `Header` and `HeaderStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `HeaderStyle`.
...
- Styling is controlled via the theme and `HeaderStyle` (where applicable).
