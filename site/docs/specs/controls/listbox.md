...
This document captures design and implementation notes for `ListBox`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `ListBox` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `ListBox` and `ListBoxStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `ListBoxStyle`.
...
- Styling is controlled via the theme and `ListBoxStyle` (where applicable).
