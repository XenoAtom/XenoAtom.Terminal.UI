...
This document captures design and implementation notes for `Slider`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `Slider` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `Slider` and `SliderStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `SliderStyle`.
...
- Styling is controlled via the theme and `SliderStyle` (where applicable).
