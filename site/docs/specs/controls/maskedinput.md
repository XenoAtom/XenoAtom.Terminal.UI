...
This document captures design and implementation notes for `MaskedInput`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `MaskedInput` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `MaskedInput` and `MaskedInputStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `MaskedInputStyle`.
...
- Styling is controlled via the theme and `MaskedInputStyle` (where applicable).
