...
- **Status**: Implemented
- **Primary purpose**: Provide `Button` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `Button` and `ButtonStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `ButtonStyle`.
...
- Styling is controlled via the theme and `ButtonStyle` (where applicable).
