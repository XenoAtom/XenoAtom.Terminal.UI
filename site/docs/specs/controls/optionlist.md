...
This document captures design and implementation notes for `OptionList`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `OptionList` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `OptionList` and `OptionListStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `OptionListStyle`.
...
- Styling is controlled via the theme and `OptionListStyle` (where applicable).
