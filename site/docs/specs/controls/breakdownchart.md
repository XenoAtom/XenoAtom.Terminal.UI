...
- **Status**: Implemented
- **Primary purpose**: Provide `BreakdownChart` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `BreakdownChart` and `BreakdownChartStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `BreakdownChartStyle`.
...
- Styling is controlled via the theme and `BreakdownChartStyle` (where applicable).
