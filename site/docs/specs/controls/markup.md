...
This document captures design and implementation notes for `Markup`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `Markup` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `Markup` and `MarkupStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `MarkupStyle`.
...
- Styling is controlled via the theme and `MarkupStyle` (where applicable).
