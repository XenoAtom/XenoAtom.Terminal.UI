---
title: Context menus Specs
---

# Context menus Specs

This document captures design and implementation notes for `Context menus`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `Context menus` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `Context menus` and `Context menusStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `Context menusStyle`.
...
- Styling is controlled via the theme and `Context menusStyle` (where applicable).
