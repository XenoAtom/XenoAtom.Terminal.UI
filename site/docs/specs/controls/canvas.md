...
- **Status**: Implemented
- **Primary purpose**: Provide `Canvas` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `Canvas` and `CanvasStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `CanvasStyle`.
...
- Styling is controlled via the theme and `CanvasStyle` (where applicable).
