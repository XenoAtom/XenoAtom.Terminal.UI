...
This document captures design and implementation notes for `Popup`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `Popup` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `Popup` and `PopupStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `PopupStyle`.
...
- Styling is controlled via the theme and `PopupStyle` (where applicable).
