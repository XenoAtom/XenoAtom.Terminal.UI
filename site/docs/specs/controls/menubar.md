...
This document captures design and implementation notes for `MenuBar`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `MenuBar` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `MenuBar` and `MenuBarStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `MenuBarStyle`.
