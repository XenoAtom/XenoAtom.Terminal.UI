...
- **Status**: Implemented
- **Primary purpose**: Provide `Border` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `Border` and `BorderStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `BorderStyle`.
...
- Styling is controlled via the theme and `BorderStyle` (where applicable).
