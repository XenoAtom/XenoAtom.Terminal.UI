...
- **Status**: Implemented
- **Primary purpose**: Provide `TextFiglet` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `TextFiglet` and `TextFigletStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `TextFigletStyle`.
...
- Styling is controlled via the theme and `TextFigletStyle` (where applicable).
