...
- **Status**: Implemented
- **Primary purpose**: Provide `TabControl` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `TabControl` and `TabControlStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `TabControlStyle`.
...
- Styling is controlled via the theme and `TabControlStyle` (where applicable).
