...
- **Status**: Implemented
- **Primary purpose**: Provide `ScrollViewer` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `ScrollViewer` and `ScrollViewerStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `ScrollViewerStyle`.
...
- Styling is controlled via the theme and `ScrollViewerStyle` (where applicable).
