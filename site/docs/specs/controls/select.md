...
This document captures design and implementation notes for `Select / Dropdown`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `Select / Dropdown` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `Select / Dropdown` and `Select / DropdownStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `Select / DropdownStyle`.
...
- Styling is controlled via the theme and `Select / DropdownStyle` (where applicable).
