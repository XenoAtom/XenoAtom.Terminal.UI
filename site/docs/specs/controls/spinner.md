...
- **Status**: Implemented
- **Primary purpose**: Provide `Spinner` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `Spinner` and `SpinnerStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `SpinnerStyle`.
...
- Styling is controlled via the theme and `SpinnerStyle` (where applicable).
