...
This document captures design and implementation notes for `ColorPicker`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `ColorPicker` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `ColorPicker` and `ColorPickerStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `ColorPickerStyle`.
...
- Styling is controlled via the theme and `ColorPickerStyle` (where applicable).
