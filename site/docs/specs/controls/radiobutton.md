...
- **Status**: Implemented
- **Primary purpose**: Provide `RadioButton` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `RadioButton` and `RadioButtonStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `RadioButtonStyle`.
...
- Styling is controlled via the theme and `RadioButtonStyle` (where applicable).
