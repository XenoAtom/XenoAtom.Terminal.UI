...
This document captures design and implementation notes for `Validation`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `Validation` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `Validation` and `ValidationStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `ValidationStyle`.
...
- Styling is controlled via the theme and `ValidationStyle` (where applicable).
