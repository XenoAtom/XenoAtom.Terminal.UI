...
This document captures design and implementation notes for `BarChart`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `BarChart` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `BarChart` and `BarChartStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `BarChartStyle`.
...
- Styling is controlled via the theme and `BarChartStyle` (where applicable).
