...
This document captures design and implementation notes for `LogControl`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `LogControl` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `LogControl` and `LogControlStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `LogControlStyle`.
...
- Styling is controlled via the theme and `LogControlStyle` (where applicable).
