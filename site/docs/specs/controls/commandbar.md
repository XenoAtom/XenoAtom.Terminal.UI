...
This document captures design and implementation notes for `CommandBar`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `CommandBar` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `CommandBar` and `CommandBarStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `CommandBarStyle`.
...
- Styling is controlled via the theme and `CommandBarStyle` (where applicable).
