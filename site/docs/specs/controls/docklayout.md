...
This document captures design and implementation notes for `DockLayout`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `DockLayout` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `DockLayout` and `DockLayoutStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `DockLayoutStyle`.
...
- Styling is controlled via the theme and `DockLayoutStyle` (where applicable).
