...
- **Status**: Implemented
- **Primary purpose**: Provide `ScrollBar` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `ScrollBar` and `ScrollBarStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `ScrollBarStyle`.
...
- Styling is controlled via the theme and `ScrollBarStyle` (where applicable).
