...
This document captures design and implementation notes for `Dialog`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `Dialog` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `Dialog` and `DialogStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `DialogStyle`.
...
- Styling is controlled via the theme and `DialogStyle` (where applicable).
