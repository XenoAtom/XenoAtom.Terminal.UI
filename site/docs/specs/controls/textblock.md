...
- **Status**: Implemented
- **Primary purpose**: Provide `TextBlock` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `TextBlock` and `TextBlockStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `TextBlockStyle`.
...
- Styling is controlled via the theme and `TextBlockStyle` (where applicable).
