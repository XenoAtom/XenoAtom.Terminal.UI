...
This document captures design and implementation notes for `SearchReplacePopup`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `SearchReplacePopup` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `SearchReplacePopup` and `SearchReplacePopupStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `SearchReplacePopupStyle`.
...
- Styling is controlled via the theme and `SearchReplacePopupStyle` (where applicable).
