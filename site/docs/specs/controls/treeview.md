...
This document captures design and implementation notes for `TreeView`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `TreeView` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `TreeView` and `TreeViewStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `TreeViewStyle`.
...
- Styling is controlled via the theme and `TreeViewStyle` (where applicable).
