...
- **Status**: Implemented
- **Primary purpose**: Provide `StatusBar` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `StatusBar` and `StatusBarStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `StatusBarStyle`.
...
- Styling is controlled via the theme and `StatusBarStyle` (where applicable).
