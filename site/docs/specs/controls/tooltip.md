...
This document captures design and implementation notes for `Tooltip`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `Tooltip` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `Tooltip` and `TooltipStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `TooltipStyle`.
...
- Styling is controlled via the theme and `TooltipStyle` (where applicable).
