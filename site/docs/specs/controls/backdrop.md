...
- **Status**: Implemented
- **Primary purpose**: Provide `Backdrop` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `Backdrop` and `BackdropStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `BackdropStyle`.
...
- Styling is controlled via the theme and `BackdropStyle` (where applicable).
