...
This document captures design and implementation notes for `Rule`.
...
- **Status**: Implemented
- **Primary purpose**: Provide `Rule` as a retained-mode control with bindable properties and predictable layout/rendering behavior.
...
- Source code lives under `src/XenoAtom.Terminal.UI` (search for `Rule` and `RuleStyle`).
...
- Uses style inheritance from the visual tree; control-specific style is typically `RuleStyle`.
...
- Styling is controlled via the theme and `RuleStyle` (where applicable).
