...
- **active animation / active interaction**
  - target up to `16.666 ms` per frame (`~60 Hz`)
...
If user code, layout, rendering, terminal I/O, or OS scheduling causes a tick to take longer than `16.666 ms` or
...
- `~60 Hz` is a sensible upper bound for smooth terminal interaction and aligns with common display/terminal refresh
  behavior.
...
- a spinner that advances every 80 ms should not wake at 16.666 ms just because the app is "animated",
- a hover or drag interaction may still benefit from the active 60 Hz cap,
