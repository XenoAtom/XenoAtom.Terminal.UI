    ...
    private static class BindableEmitter
    {
        public static void Emit(SourceProductionContext context, ImmutableArray<BindablePropertyResult> items)
        {
            ...
            foreach (var group in grouped)
            {
                ...
                var source = GenerateBindingsSource(containingType, ordered, typesWithBindings);
        ...
        private static string GenerateBindingsSource(INamedTypeSymbol containingType, List<BindablePropertyInfo> properties, HashSet<INamedTypeSymbol> typesWithBindings)
        {
            var sb = new StringBuilder(8 * 1024);
            sb.AppendLine("// <auto-generated/>");
            sb.AppendLine("#nullable enable");
        ...
        private static string GenerateRoutedEventFluentExtensionsSource(INamedTypeSymbol containingType, List<RoutedEventMethodInfo> events)
        {
            ...
            foreach (var ev in events)
            {
                ...
                if (canUseGeneric)
                {
                    ...
                    sb.Append(methodIndent).Append("    => global::XenoAtom.Terminal.UI.VisualExtensions.With(obj, x => { x.").Append(escapedRoutedHandlerName).AppendLine(" += handler; });");
                }
                else
                {
                ...
                if (canUseGeneric)
                {
                    ...
                    sb.Append(methodIndent).Append("    => global::XenoAtom.Terminal.UI.VisualExtensions.With(obj, x => { x.").Append(escapedRoutedHandlerName).Append(" += (_, __) => handler(); });").AppendLine();
                }
                else
                {
